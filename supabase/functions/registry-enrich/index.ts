...
const PLATFORM_HOSTS: Array<[RegExp, string]> = [
  ...
  [/(^|\.)vancopayments\.com$|(^|\.)vanco\.com$|(^|\.)securegive\.com$/i, "vanco"],
  [/(^|\.)planningcenteronline\.com$|(^|\.)churchcenter\.com$/i, "planning_center"],
  [/(^|\.)easytithe\.com$/i, "easytithe"],
  [/(^|\.)donorbox\.org$/i, "donorbox"],
  [/(^|\.)generis\.com$/i, "generis"],
  [/(^|\.)breezechms\.com$/i, "breeze"],
  [/(^|\.)every\.org$/i, "every_org"],
  [/(^|\.)stripe\.com$|(^|\.)checkout\.stripe\.com$|(^|\.)buy\.stripe\.com$/i, "stripe_direct"],
...
const GIVE_TEXT = /\b(give|giving|donate|donation|tithe|tithing|generosity|contribute|offering)\b/i;
const GIVE_PATH = /\/(give|giving|donate|donation|tithe|tithing|generosity|contribute|pledge)(\/|$|\?)/i;

async function firecrawlScrape(url: string) {
  const res = await fetch("https://api.firecrawl.dev/v2/scrape", {
    method: "POST",
    headers: { Authorization: `Bearer ${FIRECRAWL_API_KEY}`, "Content-Type": "application/json" },
    body: JSON.stringify({ url, formats: ["links", "html"], onlyMainContent: false, waitFor: 500 }),
  });
  if (!res.ok) throw new Error(`firecrawl ${res.status}: ${(await res.text()).slice(0, 200)}`);
  const j = await res.json();
  const d = j?.data ?? j;
  return { links: (d?.links ?? []) as string[], html: (d?.html ?? "") as string };
}
 ...
 */
async function findGivingLink(website: string): Promise<{ url: string; platform: string | null } | null> {
  const home = await firecrawlScrape(website);
  const anchors = extractAnchors(home.html);
  const siteHost = normHost(website);

  const scored: Array<{ url: string; score: number; platform: string | null; sameSite: boolean }> = [];
  ...
  for (const url of home.links) {
    ...
    scored.push({ url, score: 100, platform, sameSite: normHost(url) === siteHost });
  }

  scored.sort((a, b) => b.score - a.score);
  // Best: a platform match anywhere.
  const platformHit = scored.find((s) => s.platform);
  if (platformHit) return { url: platformHit.url, platform: platformHit.platform };
  ...
  try {
    ...
    for (const c of churches) {
      ...
      try {
        ...
        patch.enrichment_last_error = String((e as Error).message).slice(0, 500);
      }
      const { error: upErr } = await admin.from("churches").update(patch).eq("id", c.id);
      results.push({ id: c.id, name: c.legal_name, ...(upErr ? { error: upErr.message } : patch) });
    }
